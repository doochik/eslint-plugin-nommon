                    ...
                    if (firstArgument.expressions.every(item => item.type === "Identifier")) {
                        context.report({
                            ...
                            fix(fixer) {
                                ...
                                const varsToObject = firstArgument.expressions.map(exressionNode => exressionNode.name);

                                const replaceText = sourceCode
                                    .replace(/\.\${\s*([a-z0-9_]+)\s*}/gi, (match, capture) => `[${capture}]`)
                                    .replace(/\[\s*\${\s*([a-z0-9_]+)\s*}\s*]/gi, (match, capture) => `[${capture}]`)
                                    .replace(/"\${\s*([a-z0-9_]+)\s*}\s*"/gi, (match, capture) => capture)
                                    .replace(
                                        /\)$/,
                                        `, { ${varsToObject.join(", ")} })`
                                    );
